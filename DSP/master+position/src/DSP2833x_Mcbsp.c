...
//#define HOLD  GpioDataRegs.GPADAT.bit.GPIO13
//#define WP GpioDataRegs.GPADAT.bit.GPIO11
//---------------------------------------------------------------------------
...
// McBSP-A register settings
    ...
    McbspaRegs.PCR.bit.CLKXM = 1;    //1 主机 0 从机   // CLKX generated internally, CLKR derived from an external source
    ...
    McbspaRegs.SRGR1.bit.CLKGDV = CLKGDV_VAL;   // CLKG frequency = LSPCLK/(CLKGDV+1)      //时钟频率？
...
void init_mcbspb_spi()
{
    ...
    McbspbRegs.SPCR1.bit.DLB = 0;         //返回


    McbspbRegs.SPCR1.bit.CLKSTP =2;     // Together with CLKXP/CLKRP determines clocking scheme
    McbspbRegs.PCR.bit.CLKXP = 0;     //发送数据上升沿采样   // CPOL = 0, CPHA = 0 rising edge no delay
    McbspbRegs.PCR.bit.CLKRP = 0;      //接收的数下采样

    McbspbRegs.PCR.bit.CLKRM = 1;//=0输入接收时钟由外部时钟驱动
    McbspbRegs.PCR.bit.CLKXM = 1;  //=0输入发送时钟由外部时钟驱动
    ...
    McbspbRegs.SRGR1.bit.FWID = 0;              // Frame Width = 1 CLKG period
    McbspbRegs.SRGR1.bit.CLKGDV = CLKGDV_VAL;   // CLKG frequency = LSPCLK/(CLKGDV+1)
 //   McbspbRegs.SRGR1.bit.CLKGDV= 0x05;//0x01;   5时约为6M时钟    // Frame Width = 1 CLKG period, CLKGDV=16
    McbspbRegs.PCR.bit.FSXM = 1;        //=0选择外部帧同步，=1选择内部帧同步信
    McbspbRegs.SRGR2.bit.FSGM = 0;       // CLKSM=1, FPER = 1 CLKG periods
    McbspbRegs.PCR.bit.FSXP = 1; //帧同步高低电平有效。1为低电平有效
    ...
    McbspbRegs.RCR1.bit.RFRLEN1= 0;     //1帧1个字
    McbspbRegs.XCR1.bit.XFRLEN1=0;

    McbspbRegs.RCR1.bit.RWDLEN1= 5;     //32 16-bit word
    McbspbRegs.XCR1.bit.XWDLEN1=5;     // 16-bit word
