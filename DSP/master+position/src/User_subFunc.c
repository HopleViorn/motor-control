...
////////////////////////////speed100avr发送/////////////////////////////
		...
		if(spdIndex<(AvrSpeedCount-1))
		{
		 ...
		 Avr100spd=Sum/AvrSpeedCount;     //发送放encoder处

		//Avr100spd=0xccccdddd;
		 Avr100spd=Avr100spd%65536;//Avr100spd%65536;
		McbspbRegs.DXR1.all= Avr100spd;                    //发送赋值
#endif
/////////////////////////////////////////////////////////////////////////////////////////////////
		MpInB = MpInA;
		McInB = McInA;
		...
		////////////////////////////////同步速度补偿////////////////////////////////////////
		   ...
		   //     if(CountSpeedLoop<5)
		  //      {
		    //        XiGeMaspeed[CountSpeedLoop]=speed;
		    //        CountSpeedLoop++;
		     ///   }
		     //   else
		     //   {
		     //       CountSpeedLoop=0;
		      //      SyncSpeed=(XiGeMaspeed[0]+XiGeMaspeed[1]+XiGeMaspeed[2]+XiGeMaspeed[3]+XiGeMaspeed[4])/5;
		            //------------------------滤波后速度发送------------------------------------
                 //                 McbspbRegs.DXR1.all=Pferr;                    //发送赋值  位于速度得出处。 先发speed
		      //                    McbspbRegs.DXR2.all=(Uint16)(( SyncSpeed&0xffff0000)>>16);
		      //                    SendEnconder=1;           //允许中断中发送encoder）

		          //  speed3=(SyncSpeed+Slavespeed);
...
//===============================================================
	...
	else			 //不启用消抖补偿控制
	{
		...
		//speed3=(Slavespeed+speed)>>1;  //与第三轴速度速度差 ;相加/2  速度相等时第三轴速度为0  主速为正，从速为负

		e_speed = SPEED - speed;// - syncOut;       //e_speed为速度误差

	}
//=================================================================

	spdtmp = labs(e_speed);
	if(spdtmp>30000)    //差速限制 3000转
	{
		spdtmp = 30000;
	}
	if(Kisp1)
...
//-----------------------------------------
	...
	else 
	{
		...
		//-------------同速补偿----------------------------------------------------
		        ...
		        //speed3=(Masterspeed+speed%65536)>>1;

		//syncOut=AvrspdPid(speed3);
		/////////////////////////////////////
		//if(syncOut>100)syncOut=100;
		//if(syncOut<-100)syncOut=-100;
		//Iqrbuf = spdout-syncOut;      //小心电流过载
		Iqrbuf = spdout;//-syncOut;
		////////////////////////////////////
		// 转矩前馈
