...
//==============================================================================
//�����ж�
//ʹ�õ��ı������京��:RX_type,RX_end,RX_valid,Reply,Message_valid,
//==============================================================================
void SCI_ISR_Modbus(void)			
{
	Timers.SCIRxfreecnt = 0;			// SCI���ܿ��м�������0
	sci_flag.bit.RX_have_data = 1;		// ���ܵ�����
...
{	    	
	...
	if(sci_int_flag.bit.RX_type)				//232ͨѶ���մ���
	{
		if(Timers.SCIRxfreecnt >= 20000)		//2sû�����ݣ���ǰҲû�н��յ�����β��־
		{
			//sci_flag.bit.RX_end = 1;			//���ս���
			//sci_flag.bit.RX_valid = 0;			//�յ���������Ч
			//sci_flag.bit.Reply = 0;				//��������Ӧ��
			...
			//RXbuffer.pcurrent = RXbuffer.paddress;
			//sci_data.sci_state = RECEIVE_DATA_SAVE;			//ͨѶ���ڽ���״̬
			sci_data.sci_state = RECEIVE_SCI_ERROR;
		}
...
//	�����������
//	MESG.RX_mesg.RX_cmd----------->��������
//	MESG.RX_mesg.RX_data_ADR------>��Ч������ʼ��ַ
//	MESG.RX_mesg.RX_data_number--->��Ч���ݳ��� �ֽ�
//*********************************************************************************
...
void SCIReceive_MOD(void)
{		
	if( sci_flag.bit.RX_end 				// �������ݽ���
		&& sci_int_flag.bit.RX_type			// 232��Ӧ��־
		&& sci_flag.bit.RX_valid)			// ����������Ч
	{	
			...
			{				
				...
				{					
					...
					* SXbuffer.pcurrent++ = 0;										//��д��λ
				                ...
				                {
				                    * SXbuffer.pcurrent++ = 0;                                      //��д��λ
				                    * SXbuffer.pcurrent++ = int_flagx.bit.SciFindInitPhase;        //找相位                              //��д��λ
				                }
				...
				else if(pdata == 0x090A)
				                     {
				                                   * SXbuffer.pcurrent++ =(myerror2&0xff00)>>8;                                      //��д��λ
				                                   * SXbuffer.pcurrent++ =myerror2&0x00ff;                                     //��д��λ
				                     }
				pdata++;
			}
		}
...
//----------------------------------------------------------------
//	 д�������жϵ�ַ���ж����ݴ�С   дһ����ַ   ��ַ1B+������0x16 1B+RegaddrH1B+RegaddrL 1B+д�Ĵ�������1B+����H1B+���ݵ�1B+��������������+2B CRC
//----------------------------------------------------------------
void CMD06(void)
{
	...
	addrtemp=sci_data.RX_data_ADR;
	temp = 0;
	temp +=(((*addrtemp++)<<8) & 0xFF00);			//���ݵ�ַ
	temp +=((*addrtemp++) & 0x0FF);					//
	
	WrDataAdd = temp & 0xFFFF;						//Ҫд�ĵ�ַ

	WrDataEdit = (((*addrtemp++)<<8) & 0xFF00);		//Ҫд������
	WrDataEdit +=((*addrtemp++) & 0x0FF);			

	if(WrDataAdd <= PnMaxNum)						// 小于873
	{
		PnFactNum = PnSwitch(WrDataAdd);
		if((PnFactNum == 0xFFFF) || ((WrDataAdd <= 858)&(WrDataAdd >= 800) && (Commpassword != UserPassword) && (WrDataAdd != 840))) //800-858后面的参数需要口令
		{// �Ƿ���ַ �� Ȩ�޲���
			sci_flag.bit.HOST_undone = 1;
			sci_data.Error_mesg = 2;
		}
	...
	else if(WrDataAdd==0x1064)  //更改旋变分辨率  已经在参数860 此处？？
	      {
	            if((WrDataEdit<4)&&(ResoverBit!=WrDataEdit))   //数据合法且不相同
	            {
	                ResoverBit=WrDataEdit;  //0 10bit;1 14bit; 3 16bit
	                //写Flash
	...
	else if(WrDataAdd==0x1065)  //更改正交分辨率多少线   已经在参数861
	{
	                   ABLine=WrDataEdit;
	}
	else
