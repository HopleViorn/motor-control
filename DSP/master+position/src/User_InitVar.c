...
// ����
	...
	IPMImax	= Pn[IPMImaxADD];		//���������ͺž���   //Pn842 mokuai电流
	memcurSamUnit = Pn[curSamUnit];	//���������ͺž���     //560*0.1mV/A  ����ϵ�� 56mV 7*8   267 电流采样系数。 267*01V/A
	...
	if(Pn[MotorTab])
	{
		...
		varFactualTn		= Pn[FactualTn];    //额定转矩  *10
		varKu				= Pn[Ku];
		...
		JMotor				= Pn[JMotorADD];
		Iqn					= (Uint32)Pn[InormalADD]; //额定电流
		Iqm					= (Uint32)Pn[ImagneticADD];  //磁电流
	...
	{
		if(Pn[MotorTab])
			Tetacnt = Pn[InitPhaseADD];//如果不转一圈，Tetacnt会从falsh中读。如果转了一圈就以当前为准
		else
