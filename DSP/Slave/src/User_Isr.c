...
//________________________________________________________
	...
	#else
	if(!int_flag3.bit.adjCurZero && Timers.cputon >= 100)		//���� ȡ11��Iu,Iv��ֵ IuIv�Ѿ���ȥһ��2048.
	{
		if(adjCurCnt<=4095)
		{
			adjCurCnt++;
	...
	#endif
	else if(int_flag3.bit.adjCurZero)
	{
	...
	Ia = -((int32)Iu * 2930)/(int16)memcurSamUnit;	// 2.5mA
...
//_________________________________________________________________________________
	...
	{		
		...
		{
		  if (encoder >= EncElect)
		    encoder = encoder - EncElect;			// ��֤������Χ
		}
		ctemp = encoder - Tetacnt;					// ���������У׼
...
//		ctemp = ctemp + virtualPuls;
		...
		ctemp = ctemp + (EncElect>>1);				// ��Ƕȼ�180��
		if (ctemp > EncElect)
		{
		  ctemp = ctemp - EncElect;
		}
		Teta = (int16) (( ((int32)ctemp)*360 / EncElect));				// �Ƕ�ת��
	}
	...
	{
		if(!state_flag2.bit.HandPhase && !sci_oper_flag.bit.sciPhaseSON)   //对相时不受过载保护
		{
			overload();
		}
		runcur_process();
...
int16 guai_A,guai_B,flag;
	...
	loadLevelTemp = (((int32)Pn[OverloadLevelADD]*Pn[OverloadLevelADD])<<12)/625;	//Pn[OverloadLevelADD]��λ�ǰٷֱ�
...
//-----------------------------------------
	if(Pn[MotorTab])
	{
		Torque = ((int32)Iqr * varFactualTn)/Iqn;
	}
	else
	{
	...
	//	watchUnspd=Unspd;
	//	McbspbRegs.DXR2.all=(Uint16)(Unspd/10);
	}
	if(memBusSel == 3)
	{
		if(membit01.bit.bit00)
