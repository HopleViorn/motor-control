...
void clkg_delay_loop(void)
{
    ...
    McbspbRegs.RCR1.bit.RWDLEN1=2;//5; //2   16-bit word     5 为32bit
       McbspbRegs.XCR1.bit.XWDLEN1=2;//5; // 2-bit word
    // enable McBSP
    delay_loop();
    ...
    void SPISLAVEOKKKK(void)  //C2000论坛
    {
    // reset McBSP and SRG
    McbspbRegs.SPCR2.bit.FRST=0;
    McbspbRegs.SPCR2.bit.GRST=0;
   ...
   // McbspbRegs.RCR1.bit.RFRLEN1=0; //1-word
    McbspbRegs.RCR1.bit.RWDLEN1=2; // 16-bit
    McbspbRegs.XCR1.bit.XFRLEN1=1; // 1 word
    McbspbRegs.XCR1.bit.XWDLEN1=2; // 16-bit
    // enable McBSP
    delay_loop();
    McbspbRegs.SPCR2.bit.GRST = 1;
    clkg_delay_loop();
