...
unsigned char NewAD2S1210_READDO(unsigned char addr)
{
  unsigned char Wtemp;
  unsigned char Rtemp;
  unsigned char s;
...
ValCon=0x70|(ResoverBit<<2)|ABLine;     //ResoverBit 0 10bit 1 14bit 2 12bit 3 16bit .ABline:  0,256 1 1024线 2 4096线，3 16384线
NewAD2S1210_WRITE(0x92,ValCon);//Set 16Bit 0x7A->12Bit  0x75 14bit  0x7F 16bit
 //   NewAD2S1210_WRITE(0x92,0x7A);//Set 16Bit 0x7A->12Bit  0x75 14bit  0x7F 16bit
  NewAD2S1210_WRITE(0x91,0x28);//Set Driver freq      datasheet Page22    10K  250*40
  NewAD2S1210_WRITE(0x8b,0x7f);//DOS
  NewAD2S1210_WRITE(0x8c,0x01);//DOS
  NewAD2S1210_WRITE(0x88,0x01);//LOS
