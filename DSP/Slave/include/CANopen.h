...
*******************************************************************************/
   ...
   #define WORD_OF_BYTE0(Data)		(Data & 0xFF)
   #define WORD_OF_BYTE1(Data)		((Data>>8) & 0xFF)
   #define WORD_OF_BYTE2(Data)		(Data & 0xFF)
   #define WORD_OF_BYTE3(Data)		((Data>>8) & 0xFF)
   //CO_RXCAN[RXC_INDEX].Data.BYTE[0]    
/*******************************************************************************
...
*******************************************************************************/
   ...
   #define CO_RXCAN_NMT       0           //2007-6-16                //0 index for NMT message
   #define CO_RXCAN_SYNC      1                                      //1 index for SYNC message
   #define CO_RXCAN_RPDO     (CO_RXCAN_SYNC+CO_NO_SYNC)              //2 start index for RPDO messages
   #define CO_RXCAN_SDO_SRV  (CO_RXCAN_RPDO+CO_NO_RPDO)              //4 start index for SDO server message (request)
   #define CO_RXCAN_SDO_CLI  (CO_RXCAN_SDO_SRV+CO_NO_SDO_SERVER)     //5 start index for SDO client message (response)
   ...
   #define CO_TXCAN_SYNC      0                                      //0 index for SYNC message
   #define CO_TXCAN_EMERG    (CO_TXCAN_SYNC+CO_NO_SYNC)              //1 index for Emergency message
   #define CO_TXCAN_TPDO     (CO_TXCAN_EMERG+CO_NO_EMERGENCY)        //2 start index for TPDO messages
   #define CO_TXCAN_SDO_SRV  (CO_TXCAN_TPDO+CO_NO_TPDO)              //4 start index for SDO server message (response)
   #define CO_TXCAN_SDO_CLI  (CO_TXCAN_SDO_SRV+CO_NO_SDO_SERVER)     //5 start index for SDO client message (request)
   #define CO_TXCAN_HB       (CO_TXCAN_SDO_CLI+CO_NO_SDO_CLIENT)     //5 index for Heartbeat message
...
extern CO_objectDictionaryEntry CO_RPDO_MAPing[][4];
extern CO_objectDictionaryEntry CO_TPDO_MAPing[][4];
